...
| **R15：`success \| right file` 仅 n=2** | 定位器 60% recall 的全部价值押在这一环上；文件给对了但补丁还是错，前面省的都白算 | **已关闭（2026-08-08 实测，`benchmarks/results/qqcode-abba-20260808-163041/`）**。skipif 单 fixture、`--cycles 3`、openai/gpt-5.6-terra、12 runs、`incident_type` 全 `None`。automatic **6/6 behavioral**，六次全 `mode_used=fastpath`、`fastpath_success=1`、`turns_used=0`，tokens 7069 / 2184 / 2179 / 2092 / 2206 / 2149。full **6/6 behavioral**，六次全 fullagent、14–24 turns，tokens 38655 / 34377 / 23953 / 29400 / 21526 / 24496。**机制侧（关键）**：六个 automatic run 的 trace 全部是 `route_layer=l0`、`files_hint_count=0`、`prefetch_hint_count=0` —— 三个 hint 通道皆空，故定位器是唯一能把代码送进 prompt 的来源，正是 `76d4fa5` 用短路对照钉住的那条路。**与 08-07 同 fixture 同入口对比**：那时 `l0`/`fh=0`/`declined`，升级到 FullAgent 花 23,168 与 24,789 tokens；现在 FastPath 直接成功。预测为「24k decline → 约 6k 成功」，实测优于预测（首次 7,069，随后五次稳定 2,092–2,206；首次偏高疑为 workspace 冷启动，**未验证，勿当事实**）。**解读边界**：full 臂同为 6/6，故这批**不能**说明 FastPath 提高正确率，只说明同样正确的前提下便宜一个数量级（约 2.2k vs 约 28.7k，0 turns vs 14–24 turns）；「FastPath 修好了 FullAgent 做不对的 fixture」是 saferepr 的结论，不是这批。`behavioral_complete` 仍非三条件收敛的产物（R11 未解，条件 1 全程为空）。附带观察：数条 full 臂收尾总结提到环境缺 `attrs`/`attr` 使 agent 自己跑不了目标测试（验收走独立 pinned venv 不受影响），说明那 14–24 turns 有一部分花在跟环境搏斗上 |
